...
require_once("Applicants/classes/db.php");
...
if(isset($_POST)) {
	...
	$sql = "SELECT Applicants_id, Applicant_fullname, email, active FROM applicants WHERE email='$email' AND password='$password'";
	...
	if($result->num_rows > 0) {
		...
		while($row = $result->fetch_assoc()) {
			...
			if($row['active'] == '0') {
				...
				$_SESSION['name'] = $row['Applicant_fullname'];
				$_SESSION['Applicants_id'] = $row['Applicants_id'];
